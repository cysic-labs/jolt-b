//! This module implements Babybear cubic extension field mod x^3-x-1
...
/// Degree 3 Babybear extension field mod x^3+2
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Serialize, Deserialize, Hash)]
pub struct BabybearExt3(pub [Babybear; 3]);

/// For a = (a1, a2, a3) and b = (b1, b2, b3)
/// The multiplication is define as
/// c := a * b = a(x) * b(x) % (x^3+2)
///    = a3*b1*x^2 + a2*b2*x^2 + a1*b3*x^2
///    + a2*b1*x   + a1*b2*x   - 2*a3*b3x
///    + a1*b1     - 2*a3*b2   - 2*a2*b3
...
fn mul_internal(a: &BabybearExt3, b: &BabybearExt3) -> BabybearExt3 {
    ...
    let c1 = a1b1 - a3b2.double() - a2b3.double();
    let c2 = a2b1 + a1b2 - a3b3.double();
    let c3 = a3b1 + a2b2 + a1b3;
    BabybearExt3([c1, c2, c3])
}

impl_extension_field!(Babybear, BabybearExt3, 3);

impl SerdeObject for BabybearExt3 {
    ...
    fn from_raw_bytes_unchecked(bytes: &[u8]) -> Self {
        Self([
            Babybear::from_raw_bytes_unchecked(bytes[..4].as_ref()),
            Babybear::from_raw_bytes_unchecked(bytes[4..8].as_ref()),
            Babybear::from_raw_bytes_unchecked(bytes[8..].as_ref()),
        ])
    }

    fn from_raw_bytes(bytes: &[u8]) -> Option<Self> {
        let a1 = match Babybear::from_raw_bytes(bytes[..4].as_ref()) {
            Some(p) => p,
            None => return None,
        };
        let a2 = match Babybear::from_raw_bytes(bytes[4..8].as_ref()) {
            Some(p) => p,
            None => return None,
        };
        let a3 = match Babybear::from_raw_bytes(bytes[8..].as_ref()) {
            Some(p) => p,
            None => return None,
        };
    ...
    fn read_raw_unchecked<R: Read>(reader: &mut R) -> Self {
        let a1 = Babybear::read_raw_unchecked(reader);
        let a2 = Babybear::read_raw_unchecked(reader);
        let a3 = Babybear::read_raw_unchecked(reader);
        Self([a1, a2, a3])
    }
    fn read_raw<R: Read>(reader: &mut R) -> std::io::Result<Self> {
        let a1 = Babybear::read_raw(reader)?;
        let a2 = Babybear::read_raw(reader)?;
        let a3 = Babybear::read_raw(reader)?;
        Ok(Self([a1, a2, a3]))
    }
...
impl Field for BabybearExt3 {
    ...
    const ZERO: Self = Self([Babybear::ZERO; 3]);

    /// The one element of the field, the multiplicative identity.
    const ONE: Self = Self([Babybear::ONE, Babybear::ZERO, Babybear::ZERO]);

    /// Returns an element chosen uniformly at random using a user-provided RNG.
    /// Note: this sampler is not constant time!
    fn random(mut rng: impl RngCore) -> Self {
        let a1 = Babybear::random(&mut rng);
        let a2 = Babybear::random(&mut rng);
        let a3 = Babybear::random(&mut rng);
...
impl ConditionallySelectable for BabybearExt3 {
    fn conditional_select(a: &Self, b: &Self, choice: Choice) -> Self {
        Self([
            Babybear::conditional_select(&a.0[0], &b.0[0], choice),
            Babybear::conditional_select(&a.0[1], &b.0[1], choice),
            Babybear::conditional_select(&a.0[2], &b.0[2], choice),
        ])
    }
}
...
impl AsMut<[u8]> for BabybearExt3 {
    fn as_mut(&mut self) -> &mut [u8] {
        let ptr = self as *mut Self as *mut u8;
        // SAFETY Self is repr(transparent) and u64 is 8 bytes wide,
        // with alignment greater than that of u8
        unsafe { std::slice::from_raw_parts_mut(ptr, 12) }
    }
}

impl AsRef<[u8]> for BabybearExt3 {
    fn as_ref(&self) -> &[u8] {
        let ptr = self as *const Self as *const u8;
        // SAFETY Self is repr(transparent) and u64 is 8 bytes wide,
        // with alignment greater than that of u8
        unsafe { std::slice::from_raw_parts(ptr, 12) }
    }
}

impl From<Babybear> for BabybearExt3 {
    fn from(a: Babybear) -> Self {
        Self([a, Babybear::ZERO, Babybear::ZERO])
    }
}

/// This represents an element of a prime field.
impl PrimeField for BabybearExt3 {
    ...
    const TWO_INV: Self = BabybearExt3([Babybear::TWO_INV, Babybear::ZERO, Babybear::ZERO]);
    ...
    const MULTIPLICATIVE_GENERATOR: Self = BabybearExt3([
        Babybear::MULTIPLICATIVE_GENERATOR,
        Babybear::ZERO,
        Babybear::ZERO,
    ]);
    ...
    const ROOT_OF_UNITY: Self =
        BabybearExt3([Babybear::ROOT_OF_UNITY, Babybear::ZERO, Babybear::ZERO]);

    /// Inverse of [`Self::ROOT_OF_UNITY`].
    const ROOT_OF_UNITY_INV: Self =
        BabybearExt3([Babybear::ROOT_OF_UNITY_INV, Babybear::ZERO, Babybear::ZERO]);

    /// Generator of the `t-order` multiplicative subgroup.
    ///
    /// It can be calculated by exponentiating [`Self::MULTIPLICATIVE_GENERATOR`] by `2^s`,
    /// where `s` is [`Self::S`].
    const DELTA: Self = BabybearExt3([Babybear::DELTA, Babybear::ZERO, Babybear::ZERO]);
...
impl FromUniformBytes<64> for BabybearExt3 {
    fn from_uniform_bytes(bytes: &[u8; 64]) -> Self {
        Self([
            <Babybear as FromUniformBytes<16>>::from_uniform_bytes(bytes[..16].try_into().unwrap()),
            <Babybear as FromUniformBytes<16>>::from_uniform_bytes(
...
impl TryInto<Babybear> for BabybearExt3 {
    /// The type returned in the event of a conversion error.
    type Error = &'static str;

    /// Performs the conversion.
    fn try_into(self) -> Result<Babybear, Self::Error> {
